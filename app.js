(async function() {
  ...
  let buildHashCopyTimer = null;

  function showBuildHash(value) {
    if (!buildHashEl) return;
    buildHashEl.textContent = value;
    buildHashEl.title = 'Click to copy';
  }

  async function copyBuildHash() {
    if (!buildHashEl) return;
    const txt = (buildHashEl.textContent || '').trim();
    if (!txt || txt === 'loading...' || txt === 'missing version.json') return;
    try {
      ...
      buildHashEl.classList.add('copied');
      buildHashEl.title = 'Copied';
      clearTimeout(buildHashCopyTimer);
      buildHashCopyTimer = setTimeout(() => {
        buildHashEl.classList.remove('copied');
        buildHashEl.title = 'Click to copy';
      }, 1000);
    } catch {}
  }
  buildHashEl?.addEventListener('click', copyBuildHash);

  async function loadBuildHash() {
    // Single source of truth: pre-generated by scripts/write-version.mjs
    try {
      const resp = await fetch('version.json?ts=' + Date.now(), { cache: 'no-store' });
      if (resp.ok) {
        const v = await resp.json();
        if (v && v.commit) {
          showBuildHash(v.commit + (v.dirty ? '+dirty' : ''));
          return;
        }
      }
    } catch {}
    showBuildHash('missing version.json');
  }
  loadBuildHash();
